...
def parse_arguments():
    arg_parser = argparse.ArgumentParser(
        description='Universal Dependencies - Dataset Conversion (Syntax)')
    arg_parser.add_argument(
        'input_path', help='path to Universal Dependencies treebank directory')
    arg_parser.add_argument(
        'task', choices=['position', 'relative_position', 'normalized_relative'], help='task identifier')
...
def load_treebanks(path):
    ...
    for tbf in sorted(os.listdir(path)):
        ...
        tb_name = os.path.splitext(tbf)[0].replace(
            '-ud-', '-').replace('_', '-')

        # load treebank
        tbf_path = os.path.join(path, tbf)
        treebank = UniversalDependencies(
            treebanks=[UniversalDependenciesTreebank.from_conllu(tbf_path, name=tbf)])
    ...
    for tb_name, treebank in treebanks.items():
        ...
        with open(split_path, 'w', encoding='utf8', newline='') as output_file:
            ...
            for sidx in range(len(treebank)):
                ...
                # remove spaces within words (e.g. FR-GSD)
                words = [w.replace(' ', '') for w in words]

                # retrieve relevant labels
                if args.task == 'position':
                    labels = treebank[sidx].get_head()
                elif args.task in ['relative_position', 'normalized_relative']:
                    heads = treebank[sidx].get_head()
                    idx = treebank[sidx].get_idx()
                    if args.task == 'relative_position':
                        labels = [h - i for i, h in zip(idx, heads)]
                    if args.task == 'normalized_relative':
                        sentence_len = len(words)
                        labels = [round((h - i) / sentence_len, 4)
        ...
        print(
            f"Saved {tb_name} with {sidx + 1} sentences and {len(label_values)} label types to '{split_path}'.")
