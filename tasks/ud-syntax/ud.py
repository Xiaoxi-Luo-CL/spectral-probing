...
class UniversalDependencies:
    ...
    def _build_index_map(self):
        ...
        for tbidx, tb in enumerate(self._treebanks):
            # [0 ... num_sentences-1]
            tb_sentence_indices = list(range(len(tb)))
            # [(tbidx, 0) ... (tbidx, num_sentences-1)]
            tb_index_map = list(
                zip([tbidx for _ in range(len(tb))], tb_sentence_indices))
            ...
            for tbf in sorted(os.listdir(tb_path)):
                ...
                if tbf.startswith('README'):
                    ...
                    with open(tbf_path, 'r', encoding='utf8') as fp:
                        readme = fp.read()
                    metadata = re.search(
                        r'[-=]+ Machine[-\s]readable metadata(.+)', readme, flags=re.DOTALL)
                    if metadata is None:
                        continue
                    ...
                    for meta_line in metadata[1].split('\n'):
                        ...
                        if len(meta_line.split(': ')) != 2:
                            continue
                        meta_key, meta_value = meta_line.split(': ')
                        tb_meta[meta_key] = meta_value

                # skip non-conllu files
                if os.path.splitext(tbf)[1] != '.conllu':
                    continue

                # load treebank
                treebank = UniversalDependenciesTreebank.from_conllu(
                    tbf_path, name=tbf, meta=tb_meta, start_idx=cursor, ud_filter=ud_filter)
    ...
    def get_relations(self, include_subtypes=False):
        ...
        for sidx in range(len(self)):
            ...
            if sentence is None:
                continue
            relations |= set(sentence.get_dependencies(
                include_subtypes=include_subtypes)[1])
            ...
            for line_idx, line in enumerate(fp):
                ...
                if line == '':
                    try:
                        ...
                        sentence = UniversalDependenciesSentence.from_conllu(
                            start_idx + len(sentences), cur_lines)
                        # if filter is set, set any sentences not matching the filter to None
                        if (ud_filter is not None) and (not ud_filter(sentence, meta)):
                            sentence = None
...
class UniversalDependenciesSentence:
        ...
        while line_idx < len(lines):
            ...
            if '-' in tkn_idx_str:
                ...
                while (line_idx + num_words + 1) < len(lines):
                    ...
                    if int(span_tkn_idx) > tkn_span[1]:
                        break
            # check for multiword token in decimal format '1; 1.1; 1.2; ... 2' or '0.1; 0.2; ... 1' (e.g. Czech data)
                ...
                tkn_span_start = float(tkn_idx_str) if re.match(
                    r'^\d+\.\d+', tkn_idx_str) else int(tkn_idx_str) + .1
            ...
            for word_line in lines[line_idx:line_idx + num_words]:
                tkn_words.append(
                    UniversalDependenciesWord.from_conllu(word_line))
            # construct and append token
            tokens.append(UniversalDependenciesToken(
                idx=tkn_span, words=tkn_words))
    ...
    def to_conllu(self, comments=True, resolve=False):
        ...
        conllu += '\n'.join([t.to_conllu(resolve=resolve)
    ...
    def get_metadata(self):
        ...
        for comment in self._comments:
            for md_pattern in md_patterns:
                ...
                metadata['unknown'] = metadata.get(
                    'unknown', []) + [comment[1:].strip()]
        return metadata
...
class UniversalDependenciesToken:
    def __init__(self, idx, words):
        ...
        # first element is token form, all following belong to potential multiword tokens
        self._words = words
...
class UniversalDependenciesWord:
    ...
    def from_conllu(line):
        ...
        idx_str, form, lemma, upostag, xpostag, feats, head, deprel, deps, misc = [
            (v if v != '_' else None) for v in line.split('\t')]
        # parse idx string (int 1, decimal 1.1 or string '1-2')
        idx = idx_str
        if re.match(r'^\d+\.\d+$', idx_str):
            idx = float(idx_str)
        elif re.match(r'^\d+$', idx_str):
            idx = int(idx_str)
    ...
    def to_conllu(self):
        ...
        feats_str = '|'.join([f'{k}={v}' for k, v in sorted(
            self.feats.items())]) if self.feats else None
        misc_str = '|'.join([f'{k}={v}' for k, v in sorted(
            self.misc.items())]) if self.misc else None
        ...
        # convert None to '_'
        conllu_values = [
            str(v) if v is not None else '_' for v in conllu_values]
    ...
    def get_form(self, empty_as_unk=True):
        ...
        # sanitize Unicode character U+FE4FA
        form = form.replace('\U000fe4fa', '')
        form = form.replace('\ue402', '')  # sanitize Unicode character U+E402
        # sanitize Unicode character U+FEFF (zero width no-break space)
        form = form.replace('\ufeff', '')
...
class UniversalDependenciesDomainFilter(UniversalDependenciesFilter):
    ...
    def __init__(self, domains, source=None, mode='include'):
        ...
        # format {'domains': ['label0', ...], 'domain_dist': np.array}
        self._source = source
        self._mode = mode
        self._cursor = -1
    ...
    def __call__(self, sentence, meta):
        ...
        # if source is provided use maximum probability assigned domain
            ...
            domains = {
                self._source['domains'][self._source['domain_dist'][self._cursor].argmax()]}
