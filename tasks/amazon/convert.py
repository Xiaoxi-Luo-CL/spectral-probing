...
def parse_arguments():
    arg_parser = argparse.ArgumentParser(
        description='Amazon Multilingual Reviews - Dataset Conversion')
    ...
    for language in languages:
        ...
        for split_name, split in amazon.items():
            ...
            split_path = os.path.join(
                args.output_path, f'{language}-{split_map[split_name]}-sentiment.csv')
            save_dataset(split_path, texts, sentiments)
            split_path = os.path.join(
                args.output_path, f'{language}-{split_map[split_name]}-topic.csv')
