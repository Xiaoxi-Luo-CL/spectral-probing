...
def parse_arguments():
    arg_parser = argparse.ArgumentParser(
        description='20 Newsgroups - Dataset Conversion')
    arg_parser.add_argument(
        'input_path', help='path to 20 Newsgroups directory (containing splits)')
    arg_parser.add_argument('output_path', help='prefix for CSV output corpus')
    arg_parser.add_argument('-vp', '--validation_proportion', type=float, default=.2,
                            help='proportion of training data to use as validation data (default: .2)')
    arg_parser.add_argument('-rs', '--random_seed', type=int,
...
def main():
    ...
    for split_path in os.listdir(args.input_path):
        ...
        print(
            f"Loaded '{split_name}' data with {len(split_data[split_name])} items.")
    ...
    print(
        f"Created random splits: {', '.join([f'{s}: {len(lines)}' for s, lines in split_data.items()])} using random seed {args.random_seed}.")
