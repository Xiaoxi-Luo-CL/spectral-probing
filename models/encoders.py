...
class PrismEncoder(nn.Module):
    ...
    def forward(self, sentences):
        ...
        if self._emb_pooling is not None:
            ...
            emb_pooled = torch.zeros(
                (emb_tokens.shape[0], 1, emb_tokens.shape[2]), device=emb_tokens.device)
            # iterate over sentences and pool relevant tokens
            for sidx in range(emb_tokens.shape[0]):
                emb_pooled[sidx, 0, :] = self._emb_pooling(
    ...
    def embed(self, sentences):
        ...
        if (not self._specials) and (self._emb_pooling is None):
            ...
            non_special_mask = ~tok_sentences['special_tokens_mask'].bool(
            ) & att_tokens  # (batch_size, max_len)
            ...
            for sidx in range(emb_tokens.shape[0]):
                ...
                non_special_embeddings[sidx,
                                       :seq_len] = emb_tokens[sidx, non_special_mask[sidx], :]
                non_special_attentions[sidx,
                                       :seq_len] = att_tokens[sidx, non_special_mask[sidx]]
    ...
    def idct(frequencies, norm=None):
        ...
        k = torch.arange(orig_shape[-1], dtype=frequencies.dtype,
                         device=frequencies.device)[None, :] * np.pi / (2 * N)
        W_r = torch.cos(k)
        W_i = torch.sin(k)
    ...
    def retrieve(self, sentences):
        ...
        for sidx, sentence in enumerate(sentences):
            ...
            sen_hash = hashlib.md5(
                ' '.join(sentence).encode('utf-8')).hexdigest()
    ...
    def cache(self, sentences, emb_tokens, att_tokens):
        ...
        for sidx, sentence in enumerate(sentences):
            ...
            sen_hash = hashlib.md5(
                ' '.join(sentence).encode('utf-8')).hexdigest()
            # store cache entry
            self._cache[sen_hash] = emb_tokens[sidx, :torch.sum(
                att_tokens[sidx]), :]  # (sen_len, emb_dim)
